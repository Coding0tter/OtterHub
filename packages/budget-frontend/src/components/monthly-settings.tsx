import { type FixedCost } from "budget-backend/types/fixed-cost";
import {
  Button,
  Card,
  Header,
...
export const MonthlySettings = () => {
  ...
  const [categoryBreakdown, { refetch: refetchBreakdown }] = createResource(
    ...
    async (type: "income" | "expense") => {
      return (await budgetApiClient.post(`/fixed-costs/categories/${type}`))
        .data;
    },
  );
  ...
  return (
    <div class="container py-4 space-y-4">
      ...
      <Card title="Income Sources" subtitle="Add your regular income sources">
              ...
              >
                {(item, index) => (
                  <tr>
                    <td>
                      <div>{item.name}</div>
                      <div class="text-sm font-thin">{item.category.name}</div>
                    </td>
        ...
        >
          ...
          <CategoryAutocomplete
            ...
            value={fixedCost()?.category?.name || ""}
            onChange={(value) => {
              setFixedCost((prev) => ({ ...prev, category: value }));
            }}
      ...
      <Card
        ...
        subtitle={`Your monthly ${showTab()}s by category`}
      >
        <ul class="list gap-4">
            ...
            {(item, index) => (
              <li class="flex flex-row justify-between">
                <div class="flex flex-row gap-4">
                  <div class="w-5 h-5 bg-info rounded-full" />
                  <div>{item.name}</div>
                </div>
                <div class="flex flex-row gap-2">
                  <div>{formatMoney(item.total)}</div>
                  <div class="font-thin">
                    {((item.total / getSum(showTab())) * 100).toFixed(2)} %
                  </div>
                </div>
              </li>
            )}
